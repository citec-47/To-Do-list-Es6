...
const DisplayData = () => {
  ...
  myData.forEach((user, i) => {
    myDataDisplay += `<div class="myitems"><li class="my-list"><input type="checkbox" class="mycheckbox" ${user.completed}>
...
myTitle.addEventListener('keys', (event) => {
  if (event.key === 'ENTER') {
    myDataAdd();
  }
});
...
mycheckbox[i].addEventListener('click', myCheckBoxValue)
}
